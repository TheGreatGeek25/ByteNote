...
public class NoteFileFilter {
	...
	public static File requestFormatUpdate(File notefile, JFXMain jfxMain) throws IOException {
		if(notefile != null && isOldFileType(notefile)) {
			Alert rename = new Alert(AlertType.CONFIRMATION);
			rename.initOwner(jfxMain.getMainStage());
...
//				Files.move(inputFile1.toPath(), newFile.toPath());
				...
				Reader reader = CFileReader.getNoteFileReader(newFile, jfxMain);
				if(reader == null) {
					return null;
				}
				reader.noteFileMain(newFile, jfxMain);
				BYNTWriter.writeDataToFile(NoteData.getCurrentData(jfxMain), newFile);

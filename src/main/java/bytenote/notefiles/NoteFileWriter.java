...
public class NoteFileWriter {
	...
	public static void writeToFile(File file, JFXMain jfxMain) throws IOException {
		if(file.getName().endsWith(".bynt")) {
			BYNTWriter.writeDataToFile(NoteData.getCurrentData(jfxMain), file);
		} else {
			if(file.exists()) {
				...
				if(line.startsWith("BYNT")) {
					...
					CFileWriter.saveNoteFile(file, NoteData.getCurrentData(jfxMain));
				}
				scan.close();
			} else {
				CFileWriter.makeDefaultNoteFile(file);

...
public class BYNTReader {
	public static <D extends NoteData> void readBYNTFile(File file, D loadOnFail, JFXMain jfxMain) throws IOException, ClassNotFoundException {
		if(file.exists()) {
			...
			switch(version) {
			case "BYNTv1.0":
				((NoteData.V1_0) ois.readObject()).load(jfxMain);
				break;
			default:
				ois.close();

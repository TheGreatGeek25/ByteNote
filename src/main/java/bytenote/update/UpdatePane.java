...
public class UpdatePane extends BorderPane {
	...
	public UpdatePane(URL updateSite, JFXMain jfxMain) {
		...
		exitButton.setOnAction(event -> UpdatePane.this.getScene().getWindow().hide());
		setTop(exitButton);
	...
	public void _run() {
		if(!setOnClose) {
			this.getScene().getWindow().setOnCloseRequest(event -> cancelButton.fire());
			setOnClose = true;
		}
		...
		if(download != null) {
			...
			if(download.getBytesLoaded() != 0) {
				long secondsLoading = Duration.between(download.getStartTime(), Instant.now()).getSeconds();
			...
			loadingLabel.setText(Math.round((float) download.getBytesLoaded() / download.getTotalBytes() * 100) +"%   "+Long.toString(download.getBytesLoaded())+"/"+Long.toString(download.getTotalBytes())+" bytes");
